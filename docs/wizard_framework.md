...
- **Extensibility** – the only touch points for new fields are `TransactionWizard::build` (descriptor/validator/defaults) and `CliApp::populate_transaction_from_form` (the mutation target). The form engine, confirmation summary, and cancellation behaviour continue to work automatically.
