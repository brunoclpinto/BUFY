...
/// Captures forecast and summary data for a specific budgeting window.
#[derive(Debug, Clone)]
...
/// Describes the valuation policy used to convert amounts within a report.
#[derive(Debug, Clone)]
...
/// Represents failures that occur when attempting to convert between currencies.
#[derive(Debug, Clone)]
...
impl CurrencyConversionError {
    pub fn unsupported_pair(from: &CurrencyCode, to: &CurrencyCode) -> Self {
        Self {
            message: format!(
...
impl Ledger {
    pub fn new(name: impl Into<String>, budget_period: LedgerBudgetPeriod) -> Self {
        let now = Utc::now();
        Self {
            id: Uuid::new_v4(),
            name: name.into(),
    ...
    pub fn add_simulation_transaction_raw(
        &mut self,
        sim_name: &str,
    ...
    pub fn exclude_transaction_in_simulation_raw(
        &mut self,
        sim_name: &str,
        transaction_id: Uuid,
    ) -> bool {
    ...
    pub fn modify_transaction_in_simulation_raw(
        &mut self,
        sim_name: &str,
    ...
    pub fn discard_simulation_raw(&mut self, sim_name: &str) -> bool {
        ...
        let removed = len_before != self.simulations.len();
        if removed {
            self.touch();
        }
        removed
    }
    fn editable_simulation(&mut self, name: &str) -> Option<&mut Simulation> {
        let sim = self.simulation_mut(name)?;
    ...
    pub fn simulation_changes_raw(&self, sim_name: &str) -> Option<&[SimulationChange]> {
        self.simulation(sim_name).map(|sim| sim.changes.as_slice())
    }
