...
fn dispatch_action(context: &mut ShellContext, action: &str, args: &[&str]) -> CommandResult {
    match action.to_ascii_lowercase().as_str() {
        ...
        "create" | "new" => handle_create(context, args),
        "enter" => handle_enter(context, args),
...
pub(super) fn handle_enter(context: &mut ShellContext, args: &[&str]) -> CommandResult {
...
fn handle_leave(context: &mut ShellContext) -> CommandResult {
...
pub(super) fn handle_apply(context: &mut ShellContext, args: &[&str]) -> CommandResult {
    let name = resolve_simulation_name(
        context,
        args.first().copied(),
...
pub(super) fn handle_discard(context: &mut ShellContext, args: &[&str]) -> CommandResult {
    let name = resolve_simulation_name(
        context,
        args.first().copied(),
...
pub(super) fn handle_workflow_action(
